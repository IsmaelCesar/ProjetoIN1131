...
def test_breaking_pmx_case3():
    print("--"*50)
    print("Breaking PMX case 3")
    print("\t when both segments are equal")

    #op = SingleTravelerX(probability=1.)
    # when the two segments are the same
    parent1 = np.array([8, 1, 9, 11, 7, 12, 13, 0, 6, 10,  2, 5,  4, 3])
    parent2 = np.array([4, 5, 1, 3,  7, 12, 13, 0, 6, 10,  9, 11, 8, 2])
    cromossome_size = len(parent1)
    start = 4
    end  = 9
